...
import hu.nwh9yb.loginapp.utils.MD5Encrypter;
...
public class LoginServiceImpl  implements ILoginService{
    ...
    public String register(UserDTO user) {
        ...
        // Jelszó titkosítása
        userEntity.setPassword(MD5Encrypter.getMD5EncryptedValue(user.getPassword()));
        userEntity.setFullName(user.getFullName());
        userRepository.save(userEntity);
        return MessageFormat.format(
                "{0} sucessfully registered with username: {1}!",user.getFullName(), user.getUsername());
    }

    @Override
    public String login(String userName, String password) {
        UserEntity userEntity = userRepository.findUserEntityByUsername(userName);
        if(userEntity.getPassword().equals(MD5Encrypter.getMD5EncryptedValue(password))){
